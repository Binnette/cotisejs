...
describe('Cotise CRUD tests', function() {
    ...
    agent.post('/auth/signin')
      ...
      agent.post('/cotises')
        ...
        agent.get('/cotises')
          ...
          (cotises[0].title).should.match('Cotise Title');
  ...
  it('should not be able to save Cotise instance if no title is provided', function(done) {
    ...
    agent.post('/auth/signin')
      ...
      agent.post('/cotises')
        ...
        (cotiseSaveRes.body.message).should.match('Please fill Cotise title');
  ...
  it('should be able to update Cotise instance if signed in', function(done) {
    agent.post('/auth/signin')
      ...
      agent.post('/cotises')
        ...
        // Update Cotise title
        cotise.title = 'WHY YOU GOTTA BE SO MEAN?';
        ...
        agent.put('/cotises/' + cotiseSaveRes.body._id)
          ...
          (cotiseUpdateRes.body.title).should.match('WHY YOU GOTTA BE SO MEAN?');
  ...
  it('should be able to get a single Cotise if not signed in', function(done) {
    ...
    cotiseObj.save(function() {
      request(app).get('/cotises/' + cotiseObj._id)
        .end(function(req, res) {
        // Set assertion
        res.body.should.be.an.Object.with.property('title', cotise.title);
